...
def eval_metric(eval_set, model):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(eval_set):
            labels, ehr, mask, txt, _, lengths, time_step, _ = data
            logits = model(ehr, lengths, time_step)
...
def main():
    ...
    parser.add_argument('--target_rate', default=0.7, type=float)
    ...
    parser.add_argument('--model', default='NoCodeSelection', choices=['NoCodeSelection', 'NoVSkip', 'NoCSkip', 'NoMemory', 'NoTime', 'NoGlobal', 'NoTarget', 'NoFollowing', 'NoGumbel', 'NoPenalty'])
...
def train(args):
    ...
    if args.model == 'NoCodeSelection':
        model = NoCodeSelection(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoVSkip':
        model = NoVSkip(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoCSkip':
        model = NoCSkip(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoMemory':
        model = NoMemory(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoTime':
        model = NoTime(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoGlobal':
        model = NoGlobal(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoTarget':
        model = NoTarget(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoFollowing':
        model = NoFollowing(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoGumbel':
        model = NoGumbel(pad_id, args.d_model, args.dropout, args.dropout_emb)
    elif args.model == 'NoPenalty':
        model = NoPenalty(pad_id, args.d_model, args.dropout, args.dropout_emb)
    else:
    ...
    for epoch_id in range(args.n_epochs):
        ...
        for i, data in enumerate(train_dataloader):
            labels, ehr, mask, txt, _, lengths, time_step, _ = data
            optim.zero_grad()
            outputs = model(ehr, lengths, time_step)
            loss = loss_func(outputs, labels)
            loss.backward()
            total_loss += (loss.item() / labels.size(0)) * args.batch_size
            # if args.max_grad_norm > 0:
            #     nn.utils.clip_grad_norm_(models.parameters(), args.max_grad_norm)
            optim.step()
            #scheduler.step()
...
def pred(args):
    ...
    with open(log_path, 'w') as fout:
        fout.write('test_auc,test_f1,test_pre,test_recall,test_pr_auc\n')
        fout.write(
